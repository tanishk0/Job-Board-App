    ...
    <div className="max-w-6xl mx-auto space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-[#E2E8F0]">
        <div className="space-y-1">
          <Link
            href="/employer/jobs"
            className="inline-flex items-center gap-1.5 text-xs font-semibold text-[#64748B] hover:text-[#6366F1] transition-colors mb-1"
          >
      ...
      {applicantsList.length === 0 ? (
        <EmptyState
          icon={Users}
          title="No applicants for this position yet"
          description="No candidates have submitted applications for this job posting yet. Share the listing link to attract talent!"
          actionLabel="View All Job Postings"
        ...
        <div className="space-y-4">
          {applicantsList.map((applicant) => (
            <Card
              key={applicant.id}
              className="space-y-5 hover:border-[#6366F1]/40 transition-colors"
            >
              {/* Top Row: Candidate Header + Status Form */}
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-5 pb-5 border-b border-[#F1F5F9]">
                <div className="flex items-start gap-4">
                  {applicant.candidateImage ? (
                    <img
                      src={applicant.candidateImage}
                      alt={applicant.candidateName}
                      className="w-12 h-12 rounded-lg object-cover border border-[#E2E8F0] shadow-xs shrink-0"
                    />
                  ) : (
                    <div className="w-12 h-12 rounded-lg bg-[#6366F1] text-white font-bold text-base flex items-center justify-center shrink-0">
                  ...
                  <div className="space-y-1">
                    ...
                    {applicant.headline && (
                      <p className="text-xs font-semibold text-[#6366F1]">{applicant.headline}</p>
                    )}

                    <div className="flex flex-wrap items-center gap-3 text-xs text-[#64748B] pt-0.5">
                ...
                <div className="bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg p-2.5 flex items-center gap-2 shrink-0">
                  <span className="text-xs font-semibold text-[#0F172A] uppercase tracking-wider">Status:</span>
                  <form action={updateApplicationStatus} className="flex items-center gap-2">
                    ...
                    <select
                      ...
                      className="px-3 py-1.5 bg-white border border-[#E2E8F0] rounded-lg text-xs font-semibold text-[#0F172A] focus:outline-none focus:ring-2 focus:ring-[#6366F1]"
                    >
              ...
              {/* Middle Section: Cover Letter */}
              {applicant.coverLetter ? (
                <div className="bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg p-4 space-y-1.5">
                ...
                <p className="text-xs text-[#94A3B8] italic">No cover letter provided by applicant.</p>
              )}
              ...
              <div className="flex flex-wrap items-center justify-between gap-4 pt-1">
                <div className="flex items-center gap-3">
                  {applicant.resumeUrl ? (
                    ...
                    <span className="text-xs text-[#94A3B8] italic">No resume attached</span>
                  )}
                </div>
                ...
                <div className="flex items-center gap-3 text-xs">
                  {applicant.githubUrl && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-1 text-[#0F172A] hover:text-[#6366F1] font-medium"
                    >
